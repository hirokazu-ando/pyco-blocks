...
window.PYCO_LESSONS["0-15"] = {
  ...
  "mode": "python",
  "article": "https://sakigake-robo.com/courses/pycoblocks/pycoblocks-part0-intro/pycoblocks-15-return-scope/",
  "toolbox": [
    "py_def",
    "py_def_args2",
    "py_return",
  ...
  "steps": [
    {
      ...
      "bodyEasy": "<p>0-14では処理を関数にまとめました。今回はその続き。結果を返す <b>return（戻り値）</b> と、変数が使える範囲 <b>スコープ</b> を整理します。</p><p>ここが分かると、部品を組み合わせて考える段階へ進めます。</p><p>「次へ ▶」を押してください。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p><b>表示する</b> は画面に出す命令です。<b>return</b> は、関数の中で作った値を <b>呼び出した場所へ返します</b>。結果を変数に入れて次に使うなら return が要ります。</p><pre>def add(a, b):\n    return a + b\nresult = add(3, 5)\nprint(result)</pre><p><code>add(3, 5)</code> の場所に、返ってきた <code>8</code> が入ります。</p>"
    },
    {
      "track": "block",
      ...
      "bodyEasy": "<p>「関数」カテゴリの <b>関数〜（引数: x）を定義する</b> を取り出し、名前を <code>square</code>、引数を <code>n</code> にして下の図のように置きましょう。中に <b>n × n を返す（return）</b> を入れます。</p><p>定義するブロックと <b>返す</b> ブロックが置けたらクリアです。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>返ってきた値を表示してみましょう。下の図のように組み立てます。</p><ol><li><code>square</code> 関数を定義し、中で <b>n × n を返す</b></li><li>その下で <b>関数 square（引数: 6）の結果</b> を表示する</li></ol><p>「▶ 実行」を押して <code>36</code> と出れば成功です。</p><pre>36</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>関数の中で作った変数は、その関数の中だけで使えます。この範囲を <b>スコープ</b>、その変数を <b>ローカル変数</b> とよびます。外へ出す値は return で返します。</p><p>また <b>return を書かない関数</b> は、実は <b>None</b>（値がないことを表す特別な値）を返します。「表示する関数」と「値を返す関数」は目的がちがいます。</p>"
    },
    {
      "track": "block",
      ...
      "bodyEasy": "<p>return しない関数の戻り値を表示すると <b>None</b> が出ます。下の図のように組み立てましょう。</p><ol><li>引数 <code>name</code> を受け取り、それを <b>表示するだけ</b>（return しない）の <code>show_name</code> 関数を作る</li><li>その戻り値を変数 <code>result</code> に入れ、<code>result</code> を表示する</li></ol><p>「▶ 実行」を押して <code>Aoi</code> のあとに <code>None</code> が出れば成功です。</p><pre>Aoi\nNone</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>次のコードで、<code>square(7)</code> の<b>戻り値</b>はいくつでしょう？表示される値を答えてください。</p><pre class=\"code-lines\">def square(n):\n    return n * n\nprint(square(7))</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>数を受け取り、その数を<b>2倍にして return する</b>関数を作りましょう。<code>5</code> を渡した結果を表示して、<code>10</code> と出れば成功です。</p><pre class=\"code-lines\">10</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p><b>表示する</b> と <b>return</b> のちがいとして正しいのはどれでしょう？</p>",
      "check": {
        "choice": {
          "options": [
    ...
    },
    {
      ...
      "bodyEasy": "<p><b>return</b> を書かない関数の戻り値は何になるでしょう？</p>",
      "check": {
        "choice": {
          "options": [
            "None",
    ...
    },
    {
      ...
      "body": "<p>数を受け取って、その計算結果を <b>return</b> する自分の関数を作ってみましょう。<b>関数の定義</b> と <b>return</b> をどちらも使えばクリアです。</p><p>組み合わせ例：</p>",
      "bodyEasy": "<p>数を受け取って、その計算結果を <b>return</b> する自分の関数を作ってみましょう。<b>関数の定義</b> と <b>return</b> をどちらも使えばクリアです。</p><p>組み合わせ例：</p>",
