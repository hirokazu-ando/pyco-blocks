...
window.PYCO_LESSONS["0-12"] = {
  ...
  "mode": "python",
  "article": "https://sakigake-robo.com/courses/pycoblocks/pycoblocks-part0-intro/pycoblocks-12-list/",
  "toolbox": [
    "var_set",
    "py_list_empty",
    "py_list_append",
  ...
  "steps": [
    {
      ...
      "bodyEasy": "<p>これまでは1つのはこ（変数）に1つの値だけ入れてきました。でも点数が10人分あると、はこを10個作るのは大変です。</p><p><b>リスト</b>＝値を順番にならべて入れる細長い箱です。for と使えば、たくさんの値を同じやり方でまとめてあつかえます。</p><p>「次へ ▶」を押してください。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>リストは値を左からならべた入れ物です。まず <b>空のリスト</b> を作り、値を <b>追加</b> します。何番目かの番号を <b>インデックス</b>（ならび順の番号）とよび、<b>0から</b> 数えます。</p><pre>fruits = []\nfruits.append(\"apple\")\nfruits.append(\"orange\")\nprint(fruits[0])</pre><p>追加は一番うしろに足します。<code>fruits[0]</code> は先頭（0番目）です。</p>"
    },
    {
      "track": "block",
      "title": "手順1：空のリストを作る",
      "body": "<p>「変数」カテゴリの <b>変数〜を〜にする</b> に、「リスト」カテゴリの <b>空のリスト [ ]</b> をはめ込み、下の図のように置きましょう。これで空のリストが入った変数ができます。</p><p>2つのブロックが置けたらクリアです。</p>",
      "bodyEasy": "<p>「変数」カテゴリの <b>変数〜を〜にする</b> に、「リスト」カテゴリの <b>空のリスト [ ]</b> をはめこみましょう。下の図のようにならべます。これで空のリストが入った変数ができます。</p><p>2つのブロックが置けたらクリアです。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>下の図のように組み立てましょう。</p><ol><li>空のリストを変数 <code>fruits</code> に入れる</li><li><b>リスト fruits に〜を追加する</b> を3つならべ、<code>apple</code>・<code>orange</code>・<code>grape</code> を追加する</li><li><b>リスト fruits の 0 番目</b> を表示する</li></ol><p>「▶ 実行」を押して <code>apple</code> と出れば成功です。</p><pre>apple</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>要素の数（こ数）は <b>リスト〜の長さ</b> で調べられます。リストが強いのは <b>for</b> と組み合わせたときです。</p><pre>for item in parts:\n    print(item)</pre><p>中身を先頭から1つずつ <code>item</code> に入れてくり返します。値がふえても同じコードで使えます。</p>"
    },
    {
      "track": "block",
      ...
      "bodyEasy": "<p>下の図のように組み立てましょう。</p><ol><li>空のリストを変数 <code>parts</code> に入れる</li><li><code>LED</code>・<code>sensor</code>・<code>motor</code> を追加する</li><li><b>変数 item ← リスト parts を順に繰り返す</b> の中で <b>item を表示する</b></li></ol><p>「▶ 実行」を押して、3つの部品名が1行ずつ出れば成功です。</p><pre>LED\nsensor\nmotor</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>次のコードを実行すると、何が表示されるでしょう？表示される値を答えてください。</p><pre class=\"code-lines\">nums = [10, 20, 30, 40]\nprint(nums[2])</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>果物を3つ入れた<b>リスト</b>を作りましょう。そして<b>先頭（0番目）</b>と<b>3番目（インデックス2）</b>の要素を表示するコードを書いて実行しましょう。</p><pre class=\"code-lines\">apple\ngrape</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>リスト <code>fruits = [\"apple\", \"orange\", \"grape\"]</code> のとき、先頭の <code>\"apple\"</code> を取り出す書き方はどれでしょう？</p>",
      "check": {
        "choice": {
          "options": [
            "fruits[0]",
    ...
    },
    {
      ...
      "bodyEasy": "<p><code>[\"LED\", \"sensor\", \"motor\"]</code> の <b>長さ</b>（要素の数）はいくつでしょう？</p>",
      "check": {
        "choice": {
          "options": [
            "3",
    ...
    },
    {
      ...
      "body": "<p>好きな言葉を3つ以上リストに追加して、for で1つずつ表示してみましょう。<b>追加する</b> と <b>順に繰り返す</b> をどちらも使えばクリアです。</p><p>組み合わせ例：</p>",
      "bodyEasy": "<p>好きな言葉を3つ以上リストに追加して、for で1つずつ表示してみましょう。<b>追加する</b> と <b>順に繰り返す</b> をどちらも使えばクリアです。</p><p>組み合わせ例：</p>",
