  ...
  "steps": [
    {
      ...
      "bodyEasy": "<p>くり返しでリストを作るコードを、<b>1行でみじかく</b> 書けるのが <b>リスト内包表記</b>（リストを1行で作る書き方）です。</p><p>ふつうのくり返し：</p><pre>squares = []\nfor i in range(1, 6):\n    squares.append(i * i)</pre><p>内包表記だと：</p><pre>squares = [i * i for i in range(1, 6)]</pre><p><code>[ 式 for 変数 in リスト ]</code> の形で、1つずつに式を当てはめた新しいリストができます。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>「リスト」カテゴリから <b>[ 式 for 変数 in リスト ]</b> のブロックを取り出して、下の図のように置きましょう。</p><p>1つ置けたらクリアです。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>下の図のように組み立てましょう。</p><ol><li><b>変数 nums</b> に <b>リスト</b> <code>[1, 2, 3, 4]</code> を入れる</li><li>内包表記の式を <code>n × n</code>、変数を <code>n</code>、もとのリストを <code>nums</code> にする</li><li><b>値を表示する</b> で結果を表示する</li></ol><p>「▶ 実行」を押して、1つずつ2乗したリストが出れば成功です。</p><pre>[1, 4, 9, 16]</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>うしろに <b>if 条件</b> を足すと、条件に合う要素だけ取り出せます。<b>[ 式 for 変数 in リスト if 条件 ]</b> のブロックを使います。</p><pre>[n for n in nums if n % 2 == 0]  →  偶数だけ</pre><p>同じ形で <b>辞書内包表記</b> <code>{キー: 値 for …}</code> や <b>セット内包表記</b> も作れます。次は条件つきをためします。</p>"
    },
    {
      "track": "block",
      "title": "手順3：偶数だけ取り出す",
      ...
      "bodyEasy": "<p>下の図のように組み立てましょう。</p><ol><li><b>変数 nums</b> に <b>リスト</b> <code>[1, 2, 3, 4, 5, 6]</code> を入れる</li><li>条件つきの内包表記で、式を <code>n</code>、変数を <code>n</code>、もとのリストを <code>nums</code>、条件を <code>n % 2 == 0</code> にする</li><li><b>値を表示する</b> で結果を表示する</li></ol><p>「▶ 実行」を押して、偶数だけのリストが出れば成功です。</p><pre>[2, 4, 6]</pre>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>ブロックが作った Python のコードを読みましょう。</p><pre class=\"code-lines\">nums = [1, 2, 3, 4, 5, 6]\nevens = [n for n in nums if n % 2 == 0]\nprint(evens)</pre><p>内包表記は<b>うしろから読む</b>とわかりやすいです。</p><ol><li><code>for n in nums</code> で <code>n</code> を1つずつ取り出す。</li><li><code>if n % 2 == 0</code> で偶数だけ残す。</li><li>先頭の <code>n</code>（式）をならべて新しいリストにする。</li></ol><p><b>for → if → 式</b> のじゅんに読むのがコツです。</p>",
    ...
    },
    {
      ...
      "bodyEasy": "<p>次のコードを実行すると、何が表示されるでしょう？数字で答えてください。</p><pre class=\"code-lines\">nums = [1, 2, 3, 4]\nresult = [n * n for n in nums]\nprint(result[2])</pre>",
      "hint": "result は各要素を2乗したリストです。番号2（先頭から3番目）の値を答えます。",
      "check": {
        "answerText": {
          "accept": [
            "9",
    ...
    },
    {
      ...
      "bodyEasy": "<p>リスト <code>[1, 2, 3, 4, 5, 6]</code> から、<b>リスト内包表記</b>を使って偶数だけを取り出して表示するコードを書いて実行しましょう。</p><pre class=\"code-lines\">[2, 4, 6]</pre>",
      "hint": "[n for n in nums if n % 2 == 0] のように、for と if を使います。",
      "check": {
        "codeRun": {
          "outputEquals": "[2, 4, 6]",
          "codeContains": [
            {
              "pattern": "\\[[^\\]]*for[^\\]]*in[^\\]]*\\]",
              "message": "実行結果は合っていますが、リスト内包表記（[ … for … in … ] の形）で書いてみましょう"
            },
            {
              "pattern": "if",
              "message": "実行結果は合っていますが、if で偶数の条件を付けてみましょう"
            }
          ],
          "codeForbids": [
            {
              "pattern": "\\[\\s*2\\s*,\\s*4\\s*,\\s*6\\s*\\]",
              "message": "実行結果は合っていますが、答えのリストを直接書かず、内包表記で作ってみましょう"
            }
          ]
        }
      }
    },
    {
      ...
      "bodyEasy": "<p><code>[n * 2 for n in [1, 2, 3]]</code> を実行すると、どんなリストになるでしょう？</p>",
      "check": {
        "choice": {
          "options": [
            "[2, 4, 6]",
    ...
    },
    {
      ...
      "bodyEasy": "<p><code>[n for n in [1, 2, 3, 4] if n &gt; 2]</code> を実行すると、どんなリストになるでしょう？</p>",
      "check": {
        "choice": {
          "options": [
            "[3, 4]",
    ...
    },
    {
      ...
      "bodyEasy": "<p>リスト内包表記を使って、好きな計算をした新しいリストを作って表示しましょう。内包表記ブロックを1つ使えばクリアです。</p><p>組み合わせ例：</p>",
